...
class OpenIdMixin(object):
    ...
    def authenticate_redirect(self, callback_uri=None, ask_for=["name", "email", "language", "username"]):
        ...
        return redirect(self._OPENID_ENDPOINT + ("&" if "?" in self._OPENID_ENDPOINT else "?") + urllib.parse.urlencode(args))
    ...
    def _on_authentication_verified(self, callback, response):
        ...
        if not ok or b"is_valid:true" not in response.content:
        ...
        for name in request.args:
            if name.startswith("openid.ns.") and request.args.get(name) == "http://openid.net/srv/ax/1.0":
                ax_ns = name[10:]
                break
